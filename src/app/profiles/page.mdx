...
The profile model contains all the information about your profiles, such as their username, avatar, and phone number. It also contains a reference to the conversation between you and the profile and information about when they were last active on Protocol.
...
<Properties>
  ...
  <Property name="display_name" type="string">
    The profile display name in the profile list. By default, this is just the
    username.
  </Property>
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -G https://api.protocol.chat/v1/profiles \
...
<Row>
  <Col>
    ...
    This endpoint allows you to add a new profile to your profile list in Protocol. To add a profile, you must provide their Castr GÜRŲ username and phone number.
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://api.protocol.chat/v1/profiles \
      -H "Authorization: Bearer {token}" \
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl https://api.protocol.chat/v1/profiles/WAz8eIbvDR60rouK \
      -H "Authorization: Bearer {token}"
    ```
...
<Row>
  <Col>
    ...
    This endpoint allows you to perform an update on a profile. Currently, the only attribute that can be updated on profiles is the `display_name` attribute which controls how a profile appears in your profile list in Protocol.
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -X PUT https://api.protocol.chat/v1/profiles/WAz8eIbvDR60rouK \
      -H "Authorization: Bearer {token}" \
...
<Row>
  <Col>
    ...
    This endpoint allows you to delete profiles from your profile list in Protocol. Note: This will also delete your conversation with the given profile.
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -X DELETE https://api.protocol.chat/v1/profiles/WAz8eIbvDR60rouK \
      -H "Authorization: Bearer {token}"
    ```
